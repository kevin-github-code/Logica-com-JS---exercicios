# Exercícios de Lógica de Programação — JavaScript Puro (Nível Difícil)

> Regras: usar apenas laços (`for`, `while`), condicionais (`if`, `switch`), operadores e métodos
> básicos de array/string (`push`, `length`, `split`, `slice`, `indexOf`, etc.).
> **Não é permitido** usar bibliotecas, algoritmos com nome próprio, recursão avançada de grafos,
> nem métodos "mágicos" que resolvam o problema sozinhos (ex: `Array.sort()` quando o exercício
> pede para você mesmo ordenar).
